    ...
    function typLabel(t){
      if (window.FrihetI18n) return FrihetI18n.typLabel(t);
    ...
    function render(data){
      ...
      const posts = Array.isArray(lastData.poster) ? lastData.poster : [];
      const meta = document.getElementById('load-meta');
      if (meta) {
        if (window.FrihetI18n) {
          meta.textContent = FrihetI18n.tf('footer.meta', { n: posts.length, date: lastData.uppdaterad || '' });
        } else {
          meta.textContent = posts.length + ' poster · ' + (lastData.uppdaterad||'') + ' · PAPER';
        }
      }
      const missing = window.FrihetI18n ? FrihetI18n.t('empty.missing') : 'saknas';
      const put = (id, list) => {
        const el = document.getElementById(id);
        if (!el) return;
        el.innerHTML = list.length ? list.map(card).join('') : '<p class="empty">' + esc(missing) + '</p>';
      };
